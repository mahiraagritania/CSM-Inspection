...
st.title("Generate Lembar Inspeksi")
            ...
            if st.button("Mulai"):
                ...
                def post_process(results):
                    ...
                    while next_item_index < len(results):
                        ...
                        if len(item) == 3 and item[0] in ['D', 'R'] and item[1] in [0]:
                        ...
                        if len(item) == 4 and item[0] in ['D', 'R'] and item[1:].isdigit():
                            next_item = results[next_item_index + 1]
                            if ',' in next_item:
                                comma_index = next_item.index(',')
                                if comma_index == len(next_item) - 2:
                                    item = item[0] + item[1:3] + ',' + item[3]
                                    next_item_index += 1
                            else:
                                pass
                ...
                if part_no_cell:
                    # Memisahkan menggunakan . atau :
                    split_part_no = re.split(r'[.:]', part_no_cell)
                    if len(split_part_no) > 1:
                        part_no = split_part_no[1].strip()
                        print("Part No:", part_no)
                    else:
                        print("Tidak ditemukan informasi 'Part No.' setelah pemisah.")
                else:
                ...
                for index, row in df.iterrows():
                    for column_index, value in enumerate(row):
                        if isinstance(value, str) and ('Material' in value):
                            material_value = df.iloc[index - 1, column_index]
                            if isinstance(material_value, str):
                                # Cari indeks awal dan akhir informasi material
                                start_index = 1  # Mulai setelah digit pertama
                                end_index = material_value.find('C') if 'C' in material_value else len(material_value)
                                material = material_value[start_index:end_index].strip()
                                break

                if material:
                    print("Material:", material)
                else:
                    print("Tidak ditemukan informasi 'Material'.")
                ...
                for index, row in df.iterrows():
                    for column_index, value in enumerate(row):
                        if isinstance(value, str) and value.startswith('Assy On'):
                            # Pisahkan informasi sebelum dan sesudah tanda (:)
                            assy_info = value.split(":")
                            if len(assy_info) > 1:
                                assy_on = assy_info[1].strip()
                                if assy_on == '':  # Jika informasi setelah ':' kosong, cari ke kanan
                                    for col in df.columns[column_index + 1:]:
                                        if not pd.isna(df.iloc[index, col]):
                                            assy_on = df.iloc[index, col].strip()
                                            break
                                break

                if assy_on:
                    print("Assy On:", assy_on)
                else:
                    print("Tidak ditemukan informasi 'Assy On'.")
                ...
                # Langkah 5: Memasukkan variabel "assy_on" pada sel E6
                sheet['E6'] = assy_on
                    ...
                    if cell_value:
                        if len(cell_value) > 6 and '+' in cell_value:
                            ...
                            if cell_value.startswith("(") and cell_value.endswith(")"):
                                # Mengambil nilai di dalam tanda kurung
                                match = re.search(r'\d*\.?\d+', cell_value)
                                if match:
                                    dimensi = float(match.group())
                                else:
                                    dimensi = 0
                                ...
                                # Mengambil nilai toleransi dari kolom yang sesuai
                                tolerance_value = wb_toleransi['Sheet1'][f'{column_index}{tolerance_row}'].value
                                
                                # Menuliskan nilai toleransi, dimensi + toleransi, dan dimensi - toleransi ke dalam sel yang sesuai
                                sheet['C{}'.format(i)] = tolerance_value
                                sheet['D{}'.format(i)] = dimensi + tolerance_value
                                sheet['E{}'.format(i)] = dimensi - tolerance_value
